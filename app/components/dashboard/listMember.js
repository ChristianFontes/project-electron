...
export default class ListMember extends Component {
  ...
  getListUser = () => {
    ...
    .then(function(data) {
      ...
      //    if (data[key].typeMember == 'Empleado') {
      //      arrayFilter.push(data[key]);
      //    }
      // });
          ...
          <TableBody>
            {
              arrayMembers.map( (row, index) => (
                <TableRow key={index}>
                  ...
                  <TableRowColumn>{arrayMembers[index].departament[0].name}</TableRowColumn>
                  <TableRowColumn>{arrayMembers[index].sede[0].name}</TableRowColumn>
                </TableRow>
              ))
            }
          </TableBody>
