...
import { departament } from '../../api/constants';
...
export default class InputField extends Component {
  ...
  constructor(props) {
   ...
   if (this.props.create) {
     showButton = 'create';
     titleAppBar = 'Crear un nuevo Departamento'
   }
  ...
  buttonAction = () => {
    if (this.state.showButton == 'create') {
      ...
      return(
        <div style={style}>
          <RaisedButton label="Crear Departamento" primary={true} onClick={::this.handleCreate}/>
        </div>
      )
    }
    return null;
  ...
  showTicketCreated = () => {
    ...
    if (this.state.ticketCreated !== undefined || this.state.ticketCreated !== null ) {
      ...
      return (
        <h3 style={style}>Departamento creado con exito</h3>
      )
    }
    return null;
  }
